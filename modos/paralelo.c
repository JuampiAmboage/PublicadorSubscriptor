...
void* subscriber(void* args) {
    ...
    sem_wait(&selectedTopic->messageSem);
    sem_wait(&selectedTopic->mutex);
...
void* topicHandler(void* args) {
    Topic* topic = (Topic*)args;

    while (1) {
        sem_wait(&topic->messageSem);
        sem_wait(&topic->mutex);
        Message message = topic->messages[topic->messageIndex];
        topic->messageIndex++;
        sem_post(&topic->mutex);

        int i;
        for (i = 0; i < topic->messageCount; i++) {
            Subscriber subscriber = topic->subscribers[i];
